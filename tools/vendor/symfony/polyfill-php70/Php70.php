...
namespace Deconf\SEIWP\Symfony\Polyfill\Php70;
...
final class Php70
{
    public static function intdiv($dividend, $divisor)
    {
        ...
        if (0 === $divisor) {
            throw new \Deconf_SEIWP_DivisionByZeroError('Division by zero');
        }
        if (-1 === $divisor && ~PHP_INT_MAX === $dividend) {
            throw new \Deconf_SEIWP_ArithmeticError('Division of PHP_INT_MIN by -1 is not an integer');
        }
    ...
    private static function intArg($value, $caller, $pos)
    {
        ...
        if (!\is_numeric($value) || PHP_INT_MAX <= ($value += 0) || ~PHP_INT_MAX >= $value) {
            throw new \Deconf_SEIWP_TypeError(sprintf('%s() expects parameter %d to be integer, %s given', $caller, $pos, \gettype($value)));
        }
