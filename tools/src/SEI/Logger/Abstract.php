 ...
 */
abstract class SEI_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param SEI_Client $client  The current Google client
   */
  public function __construct(SEI_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('SEI_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('SEI_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('SEI_Logger_Abstract', 'date_format');
   ...
   * @throws SEI_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new SEI_Logger_Exception(
