 ...
 */
class SEI_Logger_File extends SEI_Logger_Abstract
{
  /**
  ...
  public function __construct(SEI_Client $client)
  {
    ...
    $file = $client->getClassConfig('SEI_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('SEI_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('SEI_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new SEI_Logger_Exception('File pointer is no longer available');
    }
